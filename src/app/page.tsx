    ...
    try {
      ...
      if (tickersToFetch.length > 0) {
        try {
          ...
          const res = await fetch(`/api/price?tickers=${query}`, { cache: 'no-store' });
          const data = await res.json();

          if (Array.isArray(data)) {
            data.forEach((item: any) => {
              priceMap.set(item.ticker.toUpperCase().trim(), item);
            });
          }
        } catch (e) {
      ...
      assets.forEach(asset => {
        ...
        if ((asset.type === 'stock' || asset.type === 'crypto') && asset.ticker) {
          const data = priceMap.get(asset.ticker.toUpperCase().trim());
          if (data && data.price) {
